...
def load_websites():
    """Load the 89 websites from jewelry_sites.csv"""
    import csv
    import os
    
    websites = []
    csv_path = 'jewelry_sites.csv'
    ...
    # Fallback if CSV doesn't exist or is empty
    if not websites:
        websites = [
            "https://porijewelry.com",  # Your test site
            "https://shopify-store-1.com",
            "https://shopify-store-2.com",
            "https://shopify-store-3.com",
            "https://shopify-store-4.com",
            "https://shopify-store-5.com",
        ]
...
def main():
    ...
    if analyze_button and url:
        try:
            ...
            st.subheader("🕷️ **DEEP WEB ANALYSIS**")
            st.info("🔍 **Extracting comprehensive business intelligence...**")
    ...
    # Generate Lead logic
    if generate_lead_button:
        try:
            # Load websites
            websites = load_websites()
            
            if not websites:
                st.error("❌ No websites found in the database!")
                return
            
            # Dramatic scanning effect
            st.subheader("🔍 Scanning the Internet...")
            ...
            # Scanning messages
            scanning_messages = [
                "🌐 Scanning global jewelry markets...",
                "🔍 Analyzing e-commerce platforms...",
                "📊 Evaluating lead potential...",
                "🎯 Identifying high-value targets...",
                "💎 Discovering premium jewelry brands...",
                "🛒 Mapping multi-channel retailers...",
                "📱 Detecting social media presence...",
                "🏭 Analyzing vertical integration...",
                "💰 Assessing price point strategies...",
                "🎪 Finalizing lead selection..."
            ]
            
            # Show scanning progress
            for i, message in enumerate(scanning_messages):
                status_text.text(message)
                progress_bar.progress((i + 1) / len(scanning_messages))
                time.sleep(1)  # 1 second per message = 10 seconds total
            
            # Get random website
            random_url = random.choice(websites)
            
            # Reveal the website with dramatic effect
            status_text.empty()
            progress_bar.empty()
            
            st.success("🎯 **LEAD DISCOVERED!**")
            st.markdown(f"### 🌐 **{random_url}**")
            st.info("🔍 **Analyzing lead potential...**")
            ...
            with st.spinner("🕷️ Crawling website pages..."):
                crawler = WebsiteCrawler(random_url, max_pages=max_pages)
                crawler.crawl(progress_callback=progress_callback)
            ...
            st.download_button(
                label="📄 Download JSON Results",
                data=json_str,
                file_name=f"lead_analysis_{random_url.replace('://', '_').replace('/', '_')}.json",
                mime="application/json"
            )
            ...
            if os.path.exists('website_context.txt'):
                ...
                st.download_button(
                    label="📄 Download Scraped Content",
                    data=context_data,
                    file_name=f"website_context_{random_url.replace('://', '_').replace('/', '_')}.txt",
                    mime="text/plain"
                )
        
        except Exception as e:
            st.error(f"❌ An error occurred: {str(e)}")
            st.error("Please check your API key, then try again.")
